/**
 * //可用的数据类型
 * int 储存普通的整数，默认为32位大小。
 * double 储存稍大的浮点数。
 * float 储存稍小的浮点数。
 * char 储存单字节字符。
 * void 表示“无类型”，用于声明不返回任何东西的函数，或者所指类型不明的指针，例如void *thing。
 * enum 枚举类型，类似于整数，也可转换为整数，但是通过符号化的名称访问或设置。当switch语句中没有覆盖到所有枚举的元素时，一些编译器会发出警告。
 *
 * //类型修饰符
 ...
 *
 * //类型转换
 * C使用了一种“阶梯形类型提升”的机制，它会观察运算符两边的变量，并且在运算之前将较小边的变量转换为较大边。这个过程按照如下顺序(由高到小)：
 * long duble
 * double
 * float
 * long long
 ...
 *
 * //类型大小
 * stdint.h为定长的整数类型定义了一些typedef，同时也有一些用于这些类型的宏。这比老的limits.h更加易于使用，因为它是不变的。这些类
 * 型如下：
 * int8_t 8位符号整数。
 * uint8_t 8位无符号整数。
 * int16_t 16位符号整数。
 ...
 * INT(N)_MAX N位符号整数的最大正值，例如INT16_MAX。
 * INT(N)_MIN N位符号整数的最小负值。
 * UINT(N)_MAX N位无符号整数的最大正值。为什么不定义其最小值，是因为最小值是0，不可能出现负值。
 * 在stdint.h中，对于size_t类型和足够存放指针的整数也有一些宏定义，以及其它便捷类型的宏定义。编译器至少要保证它们为某一大小，
 * 并允许它们为更大的大小。
 * int_least(N)_t 至少N位的整数。
 * uint_least(N)_t 至少N位的无符号整数。
 * INT_LEAST(N)_MAX int_least(N)_t类型的最大值。
 * INT_LEAST(N)_MIN int_least(N)_t类型的最小值。
 * UINT_LEAST(N)_MAX uint_least(N)_t的最大值。
 * int_fast(N)_t 与int_least(N)_t相似，但是是至少N位的“最快”整数。
 * uint_fast(N)_t 至少N位的“最快”无符号整数。
 * INT_FAST(N)_MAX int_fast(N)_t的最大值。
 * INT_FAST(N)_MIN int_fast(N)_t的最小值。
 * UINT_FAST(N)_MAX uint_fast(N)_t的最大值。
 ...
 * break  放在循环中用于跳出循环。
 * continue 跳到循环尾。
 * goto 跳到你已经放置label的位置，你已经在dbg.h中看到它了，用于跳到error标签。
 * / 
